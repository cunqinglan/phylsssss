...
class techmap_command : public command {
 public:
  explicit techmap_command(const environment::ptr& env)
    ...
    add_flag("--Incremental, -i", "incremental mapping");
 ...
 protected:
  void execute() {
    ...
    call_with_stopwatch(time, [&]() {
      ...
      // else if (is_set("incremental")){
      //   if (store<aig_network>().size() == 0u) {
      //     std::cerr << "[e] no AIG in the store\n";
      //   } else {
      //     std::string AND_genlib = "/home/panhongxiang/phyLS/build/bin/genlib_file/crilib.genlib";
      ...
      //       auto res = mockturtle::map(aig, lib, AND_genlib, np, ps, &st);
