...
template <>
void write_def<mockturtle::binding_view<mockturtle::klut_network>>(mockturtle::binding_view<mockturtle::klut_network> const& ntk, \
...
//   cout << "num_gates: " << ntk.num_gates() << endl;
  ...
  ntk.foreach_gate([&](auto const& n) {
    ...
    components << fmt::format("   - gate_{}_ {}\n", ntk.node_to_index(n), gates[ntk.get_binding_index(n)].name)
               << "      + UNPLACED ;\n";
    return true;
  });
  components << "END COMPONENTS\n"
