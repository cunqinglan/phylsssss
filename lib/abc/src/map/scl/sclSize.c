...
double Abc_SclTimePerformdouble( SC_Lib * pLib, Abc_Ntk_t * pNtk, int nTreeCRatio, int fUseWireLoads, int fShowAll, int fPrintPath, int fDumpStats )
{
    double maxDelay;
    SC_Man * p;
    p = Abc_SclManStart( pLib, pNtk, fUseWireLoads, 1, 0, nTreeCRatio );
...
double Abc_SclTimePerformdelay( SC_Lib * pLib, Abc_Ntk_t * pNtk, int nTreeCRatio, int fUseWireLoads, int fShowAll, int fPrintPath, int fDumpStats )
{
    double maxDelay;
    Abc_Ntk_t * pNtkNew = pNtk;
    if ( pNtk->nBarBufs2 > 0 )
        pNtkNew = Abc_NtkDupDfsNoBarBufs( pNtk );
    maxDelay = Abc_SclTimePerformdouble(pLib, pNtkNew, nTreeCRatio, fUseWireLoads,
                             fShowAll, fPrintPath, fDumpStats);
    if ( pNtk->nBarBufs2 > 0 )
        Abc_NtkDelete( pNtkNew );
    return maxDelay;
}
